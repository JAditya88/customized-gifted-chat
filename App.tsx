...
const App = () => {
  ...
  const [inputContainerPosition, setInputContainerPosition] = useState<
    "absolute" | "relative"
  >("absolute");
  const [chatMessage, setChatMessage] = useState<string>("");
  const [func, setFunc] = useState<"Reply" | "Edit" | "Send">("Send");
  ...
  const onLongPress = useCallback((context: any, message: IMessage) => {
    ...
    const options = ["Copy Text", "Reply", "Edit", "Cancel"];
  ...
  useEffect(() => {
    const keyboardWillShow = Keyboard.addListener("keyboardWillShow", () =>
      setInputContainerPosition("relative")
    );

    const keyboardWillHide = Keyboard.addListener("keyboardWillHide", () =>
      setInputContainerPosition("absolute")
    );
      ...
      <View style={styles.content}>
        <GiftedChat
          ...
          renderComposer={(props) => {
            return (
              <MessageInput
                messages={messagesData as IMessage[]}
                selectedMessage={selectedMessage}
                chatMessage={chatMessage}
                setChatMessage={setChatMessage}
                func={func}
                setFunc={setFunc}
                inputStyles={props.textInputStyle}
              />
            );
          }}
            ...
            return (
              <View
                style={{
                  ...
                  bottom:
                    inputContainerPosition === "relative"
                      ? Platform.select({ ios: 100, android: 50 })
                      : 0,
                  left: 0,
                  right: 0,
                }}
